...
`tools/list` at `https://gateway.pipeworx.io/buzzword-density/mcp` returns the tools in the table
...
```json
{
  "mcpServers": {
    "buzzword-density": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-buzzword-density"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-buzzword-density
```
